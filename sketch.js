...
function createPlatformsAndStars(){platforms=[];stars=[];var lowY=canvasH-groundHeight-player.h*2;if(lowY<60){lowY=60;}var lowX=Math.floor(random(0,canvasW-platformW));platforms.push({x:lowX,y:lowY,w:platformW,h:platformH});var attempts=0;while(platforms.length<numPlatforms&&attempts<1000){var px=Math.floor(random(0,canvasW-platformW));var py=Math.floor(random(40,canvasH-groundHeight-20));var ok=true;var nearestDist=1e9;for(var i=0;i<platforms.length;i++){var p=platforms[i];var vdist=Math.abs(py-p.y);if(vdist<nearestDist){nearestDist=vdist;}if(Math.abs(px-p.x)<platformW&&Math.abs(py-p.y)<4){ok=false;break;}}if(ok&&nearestDist<=player.h*2&&py<canvasH-groundHeight-10){platforms.push({x:px,y:py,w:platformW,h:platformH});}attempts++;}for(var i=0;i<platforms.length;i++){var p2=platforms[i];if(random()<0.8){var sx=p2.x+random(10,p2.w-10);var sy=p2.y-10;stars.push({x:sx,y:sy,r:6});}}for(var j=0;j<3;j++){var sx2=random(20,canvasW-20);var sy2=random(40,canvasH-groundHeight-150);stars.push({x:sx2,y:sy2,r:6});}} 
