...
function resetGame(){obstacles=[];score=0;spawnTimer=0;spawnInterval=90;speed=6;gameOver=false;player.y=groundY-player.size;player.vy=0;player.onGround=true}
...
function setup(){createCanvas(640,360);groundY=height-50;gravity=1;jumpForce=-15;player={x:80,y:groundY-40,size:40,vy:0,onGround:true};obstacles=[];spawnTimer=0;spawnInterval=90;speed=6;score=0;highScore=0;gameOver=false;textSize(20)}
function draw(){background(135,206,235);fill(80,200,120);rect(0,groundY,width,height-groundY);if(!gameOver){player.vy+=gravity;player.y+=player.vy;if(player.y>groundY-player.size){player.y=groundY-player.size;player.vy=0;player.onGround=true}spawnTimer++;if(spawnTimer>=spawnInterval){spawnObstacle();spawnTimer=0;if(spawnInterval>40){spawnInterval-=1}speed+=0.01}updateObstacles()}fill(200);rect(player.x,player.y,player.size,player.size);for(let i=0;i<obstacles.length;i++){let ob=obstacles[i];fill(120,80,80);rect(ob.x,ob.y,ob.w,ob.h)}fill(0);text('Score: '+score,10,30);text('High: '+highScore,10,55);if(gameOver){text('Game Over',width/2-60,height/2);text('Press R or Click to Restart',width/2-140,height/2+30)}}
